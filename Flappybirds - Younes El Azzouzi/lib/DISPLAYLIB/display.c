...
void writeCharToSegment(uint8_t segment, char character) {
    ...
    shift(SEGMENT_SELECT[segment], MSBFIRST);
    sbi(PORTD, LATCH_DIO);
    _delay_ms(50); //vertraging voor display show snelheid 
}

void writeCharsToSegment2(const char *characters) {
    uint8_t writeChars[3];
    ...
    for (uint8_t segment = 0; segment < 3; segment++) {
        ...
        shift(SEGMENT_SELECT[segment + 1], MSBFIRST); //skip segment 1 van vogek is daar
        sbi(PORTD, LATCH_DIO);
        _delay_ms(1); 
    }
}
